...
// Validation for combined auth login
const validateAuthLogin = (req, res, next) => {
  ...
  const schema = Joi.object({
    ...
    userType: Joi.string()
      ...
      .required()
      .messages({
        'any.only': 'User type must be either customer or driver',
        'any.required': 'User type is required'
      })
  });

  const { error, value } = schema.validate({ phone, password, userType });
  ...
  req.body = sanitizeObject({
    ...
    userType: value.userType.toLowerCase(),
  });
